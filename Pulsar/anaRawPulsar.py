...
def getArgs() :
    ...
    parser.add_argument("-f","--fileName",default="../data/J0332+5434/2024-06-13-1858_1.raw",help="File(s) to be analyzed.")
...
def plotFilterHist(array,args=None) :
    ...
    plt.title("Histogram for {0:s}".format(file))
    plt.xlabel("Value")
    plt.ylabel("Counts")
...
def plotTimeSeries(times,array,args=None) :
    print("Enter plotTimeSeries")
    plt.subplot() 
    plt.plot(times,array,'r-',label="All samples")
    plt.title("Time Series for {0:s}".format(file))
    plt.xlabel("Time (s)")
...
print("np.shape(power_time_series) = {0}".format(np.shape(power_time_series)))

times = np.linspace(0.,nRows*t_fft,nRows) + 0.0036
...
base_name = file.split("/")[-1].strip(".fb")
fig.suptitle("{0:s} {1:s} Period Scan: eps={2:.3e}".format(args.name,base_name,float(args.eps)), fontsize=16)
...
if sigma_mode :
    ...
    fig2.suptitle("{0:s} {1:s} Best Period={2:.3f} (ms)".format(args.name,base_name,1000.*best_period), fontsize=16)
