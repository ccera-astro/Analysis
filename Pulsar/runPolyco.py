...
def readpolycoeff(mjd,file_name): # Read the 'polyco.dat' file from tempo and finds the entry with the closest Tmid
    ...
    print("Leaving readpolycoeff")
    print('Best fit at ' + str(ind) + ' / ' + str(num))
    print("best_coeff={0:s} ind={1:d} num={2:d} tfreq={3}".format(str(best_coeff), ind, num, tfreq))
    return ( best_coeff, ind, num, tfreq )


def getpolycoeff(mjd, metadata, base_name) :
    print("base_name={0}".format(base_name))
    file_name = "polyco_{0:s}_{1:s}.dat".format(base_name.split("/")[-1][:-5],metadata['target'])
    print("Entering getplotcoeff mjd={0:f} file_name={1:s}".format(mjd,file_name))
    rfreq = 1.e-6*(metadata['freq'])
    pint.logging.setup(level="WARNING") 
    try :
        fp = open(file_name,'r')
        best_coeff, ind, num, tfreq = readpolycoeff(mjd,file_name)
        file_valid = best_coeff[0] == metadata['target'][1:] and ind != 1 and ind != num and (abs(tfreq-rfreq)/tfreq < 0.01)
    except :
